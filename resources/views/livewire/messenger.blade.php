...
new class extends Component {
    ...
    /**
     * @var array $cipherText
     * String for user message content
     */

    public function messageUser(array $cipherText)
    {
        if (empty($cipherText) || !$this->selectedConversationId) {
            return;
        }
        ...
        // Broadcast Send message reverb event
        broadcast(new MessageSent($message))->toOthers();
        $this->dispatch('scroll-bottom');
    }
    /**
     * E2EE: Save public key generated by the browser
     */
    public function registerPublicKey($publicKey)
    {
        $user = User::find(auth()->id());
