<div x-show="showSettings" class="fixed inset-0 z-[100] flex items-center justify-center p-4 md:p-8 backdrop-blur-md"
    ...
    <div class="absolute inset-0 bg-black/40 dark:bg-black/60 transition-colors duration-300"
        @click="showSettings = false; activeTab = 'chats'"></div>

    <div
        class="relative w-full max-w-3xl lg:max-w-5xl bg-[#1e1e21] rounded-3xl overflow-hidden shadow-[0_0_50px_rgba(0,0,0,0.2)] dark:shadow-[0_0_50px_rgba(0,0,0,0.5)] border border-[#2a2a2d] flex flex-col md:flex-row h-full lg:h-[80vh] max-h-[600px] lg:max-h-[750px] transition-colors duration-300">

        <div
            class="w-full md:w-64 bg-[#18181b] p-6 lg:p-8 border-r border-[#2a2a2d] hidden md:flex flex-col relative z-10 transition-colors duration-300">
            <h2
                class="text-2xl font-bold text-white mb-8 tracking-tight transition-colors duration-300">
                Settings</h2>
            <nav class="flex-1 flex flex-col gap-2">
                <button @click="activeTab = 'profile'"
                    :class="activeTab === 'profile' ? 'text-pink-500 dark:text-pink-400 bg-pink-500/10 border-pink-500/20' : 'text-[#a1a1aa] hover:bg-white/5 hover:text-white border-transparent'"
                ...
                <button @click="activeTab = 'appearance'"
                    :class="activeTab === 'appearance' ? 'text-pink-500 dark:text-pink-400 bg-pink-500/10 border-pink-500/20' : 'text-[#a1a1aa] hover:bg-white/5 hover:text-white border-transparent'"
                    class="flex items-center gap-3 w-full px-4 py-3.5 rounded-2xl transition-all duration-200 font-medium border text-left">
                    <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <form method="POST" action="{{ route('logout') }}"
                class="mt-auto pt-6 border-t border-[#2a2a2d] transition-colors duration-300">
                @csrf
                <button type="submit"
        ...
        <div
            class="flex-1 p-6 md:p-10 pb-28 overflow-y-auto relative bg-[#1e1e21] custom-scrollbar transition-colors duration-300">
            ...
            <div x-show="activeTab === 'profile'" x-transition:enter="transition ease-out duration-300 delay-75"
                ...
                <div>
                    <h3 class="text-2xl font-bold text-white mb-2 transition-colors duration-300">My
                        Profile</h3>
                    <p class="text-[#a1a1aa] text-sm transition-colors duration-300">Manage your
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-8">
                    <div class="relative group cursor-pointer flex-shrink-0">
                        <div
                            class="w-28 h-28 md:w-32 md:h-32 rounded-[2rem] overflow-hidden border-2 border-[#2a2a2d] shadow-xl dark:shadow-2xl transition-all duration-300 group-hover:scale-105 group-hover:border-pink-500/50">
                    ...
                    <div class="text-center sm:text-left pt-2">
                        <h4 class="text-white font-bold text-xl transition-colors duration-300">
                            {{ auth()->user()->name }}
                        </h4>
                        <p
                        ...
                        <button
                            class="mt-4 px-4 py-2 bg-white/5 hover:bg-white/10 text-white text-xs font-semibold rounded-lg transition-colors border border-[#2a2a2d]">Change
                            Avatar</button>
                    </div>
                </div>

                <div
                    class="space-y-6 bg-[#18181b] p-6 rounded-3xl border border-[#2a2a2d] transition-colors duration-300">
                    <div class="space-y-2">
                        <label
                            class="text-[12px] font-bold text-[#a1a1aa] uppercase tracking-wider flex items-center gap-2 transition-colors duration-300">
                        ...
                        <input type="text" value="{{ auth()->user()->name }}"
                            class="w-full bg-[#1e1e21] border border-[#2a2a2d] rounded-xl px-4 py-3.5 text-sm text-white focus:outline-none focus:border-pink-500/50 focus:ring-1 focus:ring-pink-500/50 transition-all shadow-sm dark:shadow-inner">
                    </div>
                    ...
                    <div class="space-y-2" x-data="{ copied: false }">
                        ...
                        <div class="flex items-center gap-3 relative">
                            <input x-ref="userTag" type="text" readonly
                                ...
                                class="flex-1 bg-[#1e1e21]/80 border border-[#2a2a2d] rounded-xl px-4 py-3.5 text-sm text-[#71717a] cursor-not-allowed shadow-inner select-all transition-colors duration-300">
                        ...
                        <p
                            class="text-[11px] text-[#71717a] mt-2 flex items-center gap-1.5 transition-colors duration-300">
            ...
            <div x-show="activeTab === 'appearance'" x-transition:enter="transition ease-out duration-300 delay-75"
                ...
                <div>
                    <h3 class="text-2xl font-bold text-white mb-2 transition-colors duration-300">
                        Appearance</h3>
                    <p class="text-[#a1a1aa] text-sm transition-colors duration-300">Customize how
                        the application looks to you.</p>
                </div>

                <div
                    class="space-y-4 bg-[#18181b] p-6 rounded-3xl border border-[#2a2a2d] transition-colors duration-300">
                    <label
                        class="text-[12px] font-bold text-[#a1a1aa] uppercase tracking-wider block mb-4 transition-colors duration-300">Theme</label>
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">

                        <button @click="$store.theme.set('light')"
                            :class="$store.theme.current === 'light' ? 'border-pink-500 bg-pink-500/10 ring-1 ring-pink-500/50' : 'border-[#2a2a2d] bg-[#1e1e21] hover:border-white/30'"
                            ...
                            <div
                             ...
                             <span
                                class="font-bold text-sm text-white transition-colors duration-300">Light
                                Mode</span>
                        ...
                        <button @click="$store.theme.set('dark')"
                            :class="$store.theme.current === 'dark' ? 'border-pink-500 bg-pink-500/10 ring-1 ring-pink-500/50' : 'border-[#2a2a2d] bg-[#1e1e21] hover:border-white/30'"
                            class="p-6 border-2 rounded-2xl text-center group transition-all duration-200 relative overflow-hidden">
                            <div
                            ...
                            <span
                                class="font-bold text-sm text-white transition-colors duration-300">Dark
                                Mode</span>
            ...
            <div
                class="absolute bottom-0 left-0 right-0 p-6 md:px-10 flex justify-end gap-3 pointer-events-none pt-12 transition-all duration-300"
                :class="$store.theme.current === 'dark' ? 'bg-gradient-to-t from-[#1e1e21] via-[#1e1e21] to-transparent' : 'bg-gradient-to-t from-[#ede0d4] via-[#ede0d4] to-[#ede0d4]/0'">
                <div class="pointer-events-auto flex gap-3 w-full sm:w-auto">
                    <button @click="showSettings = false; activeTab = 'chats'"
                        class="flex-1 sm:flex-none px-6 py-3 rounded-xl text-[#a1a1aa] font-semibold hover:bg-white/5 transition-colors border border-transparent">Cancel</button>
                    <button @click="showSettings = false; activeTab = 'chats'"
